          ...
          >
            ...
            <el-row style="justify-content: center; margin: 50px 0">
              <el-card
                ...
                style="
                  background-color: rgba(114, 122, 132, 0.25);
                  border: none;
                  border-radius: 15px;
                "
              >
...
export default {
  ...
  async mounted() {
    ...
    if (response.data.status === 200) {
      ...
      for (let i = 0; i < response.data.todaycount.length; i++) {
        let tt = response.data.todaycount[i].today_visit_regdate
          .split("-")[2]
          .substring(0, 2);
        if (Number(tt) === date) {
  ...
  methods: {
    ...
    dateBuilder(language) {
      ...
      if (language == "eng") return `${day} ${date} ${month} ${year}`;
      else if (language == "kor") return ` ${year} ${month} ${date} ${day}`;
    },
    ...
    dateFormat() {
      ...
      this.date = `${years}-${month >= 10 ? month : "0" + month}-${
        day >= 10 ? day : "0" + day
      }`;
    },
